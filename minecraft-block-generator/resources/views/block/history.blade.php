                        ...
                        <div class="p-5">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('item.download', $item->id) }}"
                                   class="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-2 rounded-lg transition-all text-xs text-center whitespace-nowrap">
                                    ⬇️ Télécharger
                                </a>
                                @auth
                                    @if(Auth::user()->identifier === $item->creator_identifier || Auth::user()->isAdmin())
                                        <a href="{{ route('item.edit', $item->id) }}"
                                           class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-3 rounded-lg transition-all text-xs">
                                            ✏️
                                        </a>
                                        <button onclick="confirmDeleteItem({{ $item->id }}, '{{ $item->name }}')"
                                                class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-3 rounded-lg transition-all text-xs">
                                            🗑️
                                        </button>
                                    @endif
                                @endauth
